...
// 必要なバッファサイズを50MBと仮定
#define WAVEFORM_SIZE (50 * 1024 * 1024) // 50MB
...
int main() {
    int ch;
    int num;
    char *binWaveform = malloc(WAVEFORM_SIZE);
    char fname_read[MAX_FILENAME];
    char fname_save[MAX_FILENAME];
    char fileBuf[512];        
    double trigtime;
    ...
    for (ch = 3; ch <= 4; ch++) {
        for (num = 0; num < 200; num++) {
            FILE *fp_r;
            FILE *fp_w;
            printf("Processing file num = %d\n", num);
            char *binWaveform = malloc(WAVEFORM_SIZE);  // 40MB のバッファを確保
            if (binWaveform == NULL) {
            ...
            WAVE_DESC wd;
            // ファイルの読み込み
            snprintf(fname_read, MAX_FILENAME, "/Users/ryutaro_matsumoto/Desktop/Reaserch/Analysis_codes/lecroy_data_converter/trc/C%d--wave--%05d.trc", ch, num);
            fp_r = fopen(fname_read, "rb");
            if (fp_r == NULL) {
                printf("Error: Can't open file %s\n", fname_read);
                continue;  // エラー時には次のファイルへ
            }

            size_t bytesRead = fread(binWaveform, 1, WAVEFORM_SIZE, fp_r);
            fclose(fp_r);

            if (bytesRead < WAVEFORM_SIZE) {
                printf("Warning: Only read %zu bytes from %s\n", bytesRead, fname_read);
            }

            // デスクリプタを取得
            if (GetDescriptor(binWaveform, &wd) != 0) {
                printf("Error: Unable to get waveform descriptor from %s\n", fname_read);
                continue;
            }

            // CSVファイルの作成
            snprintf(fname_save, MAX_FILENAME, "/Users/ryutaro_matsumoto/Desktop/Reaserch/Analysis_codes/lecroy_data_converter/csv/C%d--wave--%05d.csv", ch, num);
            ...
            int wf_length = wd.wave_array_count / wd.subarray_count;
            for (int seg = 0; seg < wd.subarray_count; seg++) {
