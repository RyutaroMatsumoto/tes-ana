...
def process_optfit(period1:int,run1:int,channel1:int,period2:int,run2:int,channel2:int, cnr, maxfreq, phmin, phmax, timin, timax, normalize, verbose, base_dir:Path)->None:
    ...
    if cnr:
        ...
        pulse = pulse - noise_ave
        noise = noise - noise_ave
        if DEBUG:
            plt.plot(pulse[100])
            plt.plot(noise[100])
            plt.title("Ramdom differentialized Pulse & Noise")
            plt.xlabel("Sample Index")
            plt.ylabel("Amplitude")
            plt.show()
            plt.plot(pulse[100])
            plt.plot(noise_ave)
            plt.title("Ramdom differentialized Pulse & Mean Noise")
            plt.xlabel("Sample Index")
            plt.ylabel("Amplitude")
            plt.show()
            plt.plot(pulse[100])
            plt.plot(np.mean(pulse,axis=0))
            plt.title("Ramdom differentialized Pulse & Mean Pulse")
            plt.xlabel("Sample Index")
            plt.ylabel("Amplitude")
            plt.show()
        average = make_average_pulse(pulse, phmin, phmax, timin, timax, normalize, verbose, False)
    ...
    plt = gaussian_fit(ph_array)
    plt.savefig(f"{plt_dir}/fitted_hist_optimal-filter_p{period1}_r{run1}.png")
    ...
    # plt.grid()
    # plt.ylabel("Counts")
    # plt.xlabel("Pulse height [a.u]") #出力の単位はV
    # plt.hist(ph_array, bins=256)
    # plt.plot(gaussian_fit(ph_array))
